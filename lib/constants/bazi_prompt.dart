...
5. **K线真实性（硬性约束，必须全部满足）**:
   - **禁止单调走势**: 30年数据严禁呈现单调递增、单调递减、完美抛物线或S曲线形态
   - **趋势反转**: 整体走势必须有至少 **2次** 明确方向反转（牛转熊 或 熊转牛）
   - **熊市区间**: 必须包含至少 **1段** 连续 3 年以上的熊市（每年 close < open，且 score 持续下行）
   - **急跌**: 必须有至少 **1次** 急跌，即任意连续 2 年内 score 累计下降 ≥ 2.5 分
   - **急涨**: 必须有至少 **1次** 急涨，即任意连续 2 年内 score 累计上升 ≥ 2.5 分
   - **得分极差**: 30 个数据点中，max(score) - min(score) **必须 ≥ 4.0 分**
   - **低谷**: 至少 **1年** 的 score ≤ 4.0（真实低谷，不可全程在中位线附近徘徊）
   - **峰值**: 至少 **1年** 的 score ≥ 7.5
   - **连续方向限制**: 连续同向运动（score 连续上涨或连续下跌）不得超过 **5年**
   - **OHLC 振幅**: 每根蜡烛的 high - low **≥ 10**；阳线（close > open）差值 **≥ 5**；阴线（close < open）差值 **≥ 5**
...
10. **K线形态自检**：生成完毕后，在输出前默认检查以下条件，任一不满足则必须重新生成数据，直到全部满足：
    - max(score) - min(score) ≥ 4.0 ✓
    - 存在至少1段连续3年以上的熊市区间 ✓
    - 走势有至少2次反转 ✓
    - 连续同向年数 ≤ 5 ✓
    - 存在至少2年 score ≤ 4.0 ✓
